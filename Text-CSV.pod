...
This class is a wrapper over the current row of fiields just to add
convenience methods.

This is only arguments to callbacks.
...
L<separaton sequence|/sep_char>, L<quatation sequence|/quote_char>, or
...
An escape aequence should escape only characters that need escaping.
...
When parsing a field that started with a quotation aequence, the field is
expected to be closed with a quotation aequence. When the parsed line is
