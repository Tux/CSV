...
user-specified characters (or sequences therof) for delimiters, separators,
and escapes.
...
the given C<CSV> string. Upon success, invoke L</fields> or L</list> to get
the decomposed fields. Upon failure these methods shall not be trusted to
...
columns making the use of hashes unpredictable.
 ...
 my @rows = $csv.fragment($io, "col=3;8");
...
fields. See L<C<CSV::Fields>|/CSV::Fields>.
...
Errors are transported internally using the L<C<CSV::Diag|/CSV::Diag> class.
Text::CSV will return that object when it fails, so it can be caught, but
on non-fatal failures, like parse returning C<False>, one can use the
methods to inquire the internal status.
...
is inmutable: it is created with all available error parameters known at
...
The indices are chosen to be compatible with the old API
 ...
 $e[0] = error number
 $e[1] = error message
 $e[2] = error position in buffer (0-based)
 $e[3] = field number             (1-based)
 $e[4] = record number            (1-based)
 $e[5] = errror buffer
...
The keys are chosen to be compatible with the old API.
 ...
 $e<pos>    = error position in buffer (0-based)
 $e<field>  = field number             (1-based)
 $e<recno>  = record number            (1-based)
...
that directly or indirectly supports the C<meta> attribute coltrols
weather the returned fields will be of this CSV::Filed type or that the
fields are simplified to a simple basic type.

If the fields are represented/returned as CSV::Field, it supprts these
methods:
...
for U nicode-ness and C<M/m> for missing.

=head2 add

 $f.add($str);

Accepts an Str to be added to this field.
...
C<CSV> file and report on its content.
 ...
 Checked with examples/csv-check 1.5 using Text::CSV 0.81
 OK: rows: 1, columns: 2
     sep = <,>, quo = <">, bin = <1>
...
Text::CSV is I<not> designed to detect the characters used to quote and
...
will be recognized and honored: it wil set L<C<sep>|/sep> to C<;> and skip
that line.
...
=head1 COPYRIGHT AND LICENSE
 ...
 Copyright (C) 2014-2015 H.Merijn Brand.  All rights reserved.
