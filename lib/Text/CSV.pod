...
separators match, set C<sep> to that sequence for the current CSV_XS
instance and use it to parse the first line, map those to foldcase, use
that to set the instance L</column_names> and return the instance:
...
The default is to fold the header using foldcase. You can also choose to
fold the headers to lower case with C<< fold => "lc" >>, upper case with
C<< fold => "uc" >> or to leave the fields as-is with C<< fold => "none" >>.

=item columns

 $csv.header($fh, :columns);
...
distinction between the two, use L<C<quote_empty>|/quote_empty>.
 ...
 csv(out => "foo.csv", in => sub { $sth.fetch }, :quote_empty);
...
=head1 COPYRIGHT AND LICENSE
 ...
 Copyright (C) 2014-2016 H.Merijn Brand. All rights reserved.
