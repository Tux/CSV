...
The Comma Separated Value (CSV) File Format"|creativyst.com/Doc/Articles/CSV/CSV01.shtml>
...
Replace the content of fields that start with a C<=> with C<Str>.
...
This function implements L<RFC7111|https://datatracker.ietf.org/doc/html/rfc7111>
...
will also return C<False> for C<"0">, where raku sees C<"0"> as C<True>.
...
writing and closed when finished, a file handle (e.g. C<$fh> or
C<IO::Handle>), a C<Channel>, a C<Supply>, a C<Callable>, or C<Nil>.
...
When C<Nil> is used for out, the output is completely suppressed. This can
be useful when streaming, and the output is not required, but the
side-effects of e.g. an C<on-in> hook is dealing with the data.
...
If C<out> points to an existing Hash, the data will be added to that instead
(existing records will be overwritten on reading duplicate keys). It returns
the value passed to C<out>:
...
If you were using a key list, I<all> keys should exist.
