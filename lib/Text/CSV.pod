...
that to set the instance column-names and return the instance:
...
folding), it will trow an exception with error 1010, 1011, 1012, or 1013
respectively.
...
fold the headers to upper case with C<< fold => "uc" >> or to leave the
fields as-is with C<< fold => "none" >>.
