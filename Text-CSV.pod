...
C<Str> but of type C<CSV::Field>, which will feature these methods:

=over 2

=item new

The contructor optionally takes an Int or a Str

=item text (Str)
=item Str (Str)

Returns the text as returned when C<keep-meta> was C<False>.

=item Bool (Bool)

Returns the boolean value of the field. As CSV is text-only be design, this
will also return C<False> for C<"0">, where perl6 sees C<"0"> as C<True>.

=item Numeric (Int/Num)

Will return the numeric value of the field is appropriate.
...
Returns C<True> when the field is undefined.

=item is_binary (Bool)

Returns true if the field has data beyond ASCII (WIP).

=item is_utf8 (Bool)

Returns C<True> if the field is beyond ASCII, but still valid UTF-8.

=item is_missing (Bool)

Returns C<True> if the field is missing (WIP).
...
C<"\cAUNDEF\cA">, so
 ...
 $csv.column_names(Str, "", "name", "name");
 $hr = $csv.getline_hr($io);
...
successful, C<error_input> will return C<Str>.
